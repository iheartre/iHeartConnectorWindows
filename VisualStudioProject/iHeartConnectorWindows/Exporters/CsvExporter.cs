    ...
    internal class CsvExporter : Exporter
    {
        ...
        protected override void DoWork()
        {
            ...
            try
            {
                ...
                sb.AppendLine($"Time{_separator}Millis{_separator}SpO2{_separator}Pulse Rate{_separator}IR{_separator}IR Index{_separator}SpO2 Status{_separator}Battery");
                for (int i = 0; i < _data.Count && !this.terminationRequested; i++)
                {
                    ...
                    sb.AppendLine($"{_data[i].Timestamp.ToString("yyyy.MM.dd HH:mm:ss.fff")}{_separator}{_data[i].Millis}{_separator}{_data[i].SpO2}{_separator}{_data[i].PulseRate}{_separator}{_data[i].IR}{_separator}{_data[i].IRIndex}{_separator}{_data[i].SpO2Status:X2}{_separator}{_data[i].BatteryLevel/1000}");
                }
